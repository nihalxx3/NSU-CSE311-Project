...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$property_exists) {
        ...
        // SQL query to insert data into inspection table
        $sql = "INSERT INTO inspection (PropertyID, InspectionDate)
                VALUES (?, ?)";

        // Use prepared statements for security
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("is", $propertyID, $inspectionDate);

        // Execute the query
        if ($stmt->execute()) {
            echo "Inspection details added successfully!";
        } else {
