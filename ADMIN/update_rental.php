...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$property_exists) {
        ...
        // SQL query to update data in rentals table
        $sql = "UPDATE rentals 
                SET AgreementDate = ?, RentAmount = ? 
                WHERE PropertyID = ? AND RentalID = ?";

        // Use prepared statements for security
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssdi", $agreementDate, $rentAmount, $propertyID, $rentalID);

        // Execute the query
        if ($stmt->execute()) {
            echo "Rental information updated successfully!";
        } else {
